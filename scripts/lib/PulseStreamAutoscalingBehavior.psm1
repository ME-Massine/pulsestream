...
function Get-RestartDelta {
    ...
    # A container's first observation is its baseline: Pods that already
    # existed before this run may legitimately carry historical restarts.
    # Thereafter retain its greatest observed count even after the pod has been
    # scaled down, so churn cannot erase a restart from the verdict.
    $baselines = @{}
    $maxima = @{}
...
function Format-AutoscalingTimeline {
    ...
    foreach ($sample in $Timeline.Samples) {
        ...
        $line = "{0} | cpu: {1,9}/{2}%  {3}  {4}  {5} | ready={6}" -f `
