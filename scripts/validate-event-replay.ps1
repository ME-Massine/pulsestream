...
#    and is now safe to reprocess. The unique event id lets us locate it through the whole flow, and
#    scopes the log/topic assertions below to this run so they cannot pass on stale state from an
...
#    leftover line from an earlier invocation of this script.
Invoke-WithRetry `
    -TimeoutSeconds $TimeoutSeconds `
