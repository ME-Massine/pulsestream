...
| :--- | :--- | :--- | :--- | :--- |
| `ingestion-service` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/ingestion-service/hpa-behavior-runtime-verification.md) | 2 → 3 → 4 → 6; peak 306% against the 70% target; all 6 replicas became Ready while the peak target remained requested | Yes; proven recommendation duration ≥277s (anchor bundle completed 01:38:45Z at 6 replicas and 10%, desiring the floor; last recommending bundle started 01:43:22Z and still showed 6; decision observed 01:43:33Z), then 6 → 5 → 4 → 3 → 2 | 7 of 7 transitions consumed exact-size `SuccessfulRescale` occurrences from HPA UID `7e03694f-fac1-4371-a5b3-3859cc590d57`; 70 complete event/scale samples, widest evidence collection 4s, heartbeat 633 → 52079, Ready ≥2, zero restarts, cleanup confirmed |
| `telemetry-processor` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/telemetry-processor/hpa-behavior-runtime-verification.md) | 2 → 3; peak 105% against the 70% target; all 3 replicas became Ready while the peak target remained requested | Yes; proven recommendation duration ≥279s (anchor bundle completed 02:14:08Z at 3 replicas and 24%, desiring the floor; last recommending bundle started 02:18:47Z and still showed 3; decision observed 02:18:58Z), then 3 → 2 | 2 of 2 transitions consumed exact-size `SuccessfulRescale` occurrences from HPA UID `50972f55-b56a-46f6-a663-c7bfc0abcadd`; 39 complete event/scale samples, widest evidence collection 1s, heartbeat 4800 → 45600, Ready ≥2, zero restarts, cleanup confirmed |

Both reports were generated by `validate-autoscaling-behavior.ps1 -IncludeScaleDown` from clean detached worktrees at logic revision `034d2da3004fc2c842865efa8554af6bc2fc297b`, against Kubernetes client/server v1.36.1. Neither HPA exposed `metadata.generation`; each report records that fact and proves that the UID and serialized spec remained fixed throughout its run. Every post-baseline `SuccessfulRescale` occurrence mapped one-to-one to a transition with the same exact `New size`, with zero unmatched occurrences.

The ingestion run used 6 concurrent request loops with a 10-second sampling interval. Telemetry used a bounded 600 events/s burst for 60 seconds, followed by 50 events/s maintenance traffic, also sampled every 10 seconds; an 800 events/s calibration attempt was rejected after the new replica restarted during startup rather than being reported as clean evidence. The generated conclusions use completion of the anchor evidence bundle through the start of the last recommending bundle, not the longer anchor-to-decision observation spacings (288s for ingestion and 290s for telemetry).
