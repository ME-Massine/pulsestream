...
The consequence is symmetric and intended: a gap around *either* transition shrinks the proven bound and fails the assertion. A run that stalled for 299 seconds between two idle post-load samples proves nothing about that stretch of the window and is rejected, not forgiven; an anchor sampled 90 seconds after its predecessor contributes exactly nothing extra. Uncertainty reduces what the evidence proves — it never adds credit.
