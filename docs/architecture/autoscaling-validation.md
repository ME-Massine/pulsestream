...
Telemetry uses one producer with a 500 events/s scale-up burst for 60 seconds, then a sustainable 300 events/s rate for the rest of the load phase. The three values are configurable with `-KafkaBurstEventsPerSecond`, `-KafkaBurstDurationSeconds`, and `-KafkaEventsPerSecond`; ingestion keeps three lightweight curl pods. Passing `-LoadPodCount` overrides either service default. This bounded shape keeps the normal run focused on HPA behavior: multiple unbounded JVM producers can create backlog faster than the processor can drain it and starve a laptop-class single-node cluster, which turns the validation into an accidental stress test. Increase the burst only when the reported peak remains below the applied CPU target.
