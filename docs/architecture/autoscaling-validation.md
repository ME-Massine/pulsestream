...
Telemetry uses one producer with a 500 events/s scale-up burst for 45 seconds, then a 50 events/s maintenance rate for the rest of the load phase. The burst moves CPU past the HPA target; the maintenance rate keeps heartbeat/liveness verification meaningful without continuing to build backlog after scale-up. The three values are configurable with `-KafkaBurstEventsPerSecond`, `-KafkaBurstDurationSeconds`, and `-KafkaEventsPerSecond`; ingestion keeps three lightweight curl pods. Passing `-LoadPodCount` overrides either service default. This bounded shape keeps the normal run focused on HPA behavior: multiple unbounded JVM producers can create backlog faster than the processor can drain it and starve a laptop-class single-node cluster, which turns the validation into an accidental stress test. Increase the burst only when the reported peak remains below the applied CPU target.
...
pwsh -File scripts/validate-autoscaling-behavior.ps1 `
    ...
    -IncludeScaleDown `
    -ReportPath infrastructure/kubernetes/telemetry-processor/hpa-runtime-verification.md
```

If the peak utilization reported at the end never crosses the target, raise `-LoadPodCount` or `-LoadConcurrency`: on a fast node a handful of `curl` loops may not saturate a `250m` request.
...
| :--- | :--- | :--- | :--- | :--- |
| `ingestion-service` | [`hpa-runtime-verification.md`](../../infrastructure/kubernetes/ingestion-service/hpa-runtime-verification.md) (#150) | 2 → 4 → 6, ceiling held at `ScalingLimited/TooManyReplicas` | Yes, ~300s window observed, then one pod per ~60s back to 2 | Two of the six replicas stayed `Pending` on a single node at 93% memory — a cluster capacity limit, not an autoscaler one |
| `telemetry-processor` | PR #283 (#151) | 2 → 3 under real Kafka traffic, all three consumer-group members holding one partition each | Yes, back to 2 with the group re-formed across 3 partitions and lag at 0 | Recorded by hand before this harness existed |

Both predate `validate-autoscaling-behavior.ps1` and were captured by running the commands manually. They establish that the behavior is correct; the harness makes it repeatable and gives it a pass/fail verdict instead of a reading exercise. Re-running it on each cluster the platform is deployed to is the intended use, with the generated report committed next to the manifest it describes.
