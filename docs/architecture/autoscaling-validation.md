...
Runtime verification is pending for the target-specific event-attribution revision. The two committed report files were produced by the superseded `desiredReplicas`/`lastScaleTime` attribution model and are not runtime proof for the current logic. They must be regenerated — not edited — by both clean-tree `-IncludeScaleDown` commands after the logic is committed, then this section must be replaced with the exact tested revision, Kubernetes client/server versions, HPA UID/generation, event evidence, readiness/restarts/heartbeat/cleanup facts, scale-up figures, and conservative scale-down bounds from those generated reports.
