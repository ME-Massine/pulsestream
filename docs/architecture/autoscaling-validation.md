...
Telemetry uses one producer capped at 300 events/s by default (`-KafkaEventsPerSecond`); ingestion keeps three lightweight curl pods. Passing `-LoadPodCount` overrides either service default. The service-specific pod count and rate cap keep the normal run focused on HPA behavior: multiple unbounded JVM producers can create backlog faster than the processor can drain it and starve a laptop-class single-node cluster, which turns the validation into an accidental stress test. Increase the rate or pod count only when the reported peak remains below the applied CPU target.
