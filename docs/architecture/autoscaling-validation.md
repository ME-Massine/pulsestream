...
| :--- | :--- | :--- | :--- | :--- |
| `ingestion-service` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/ingestion-service/hpa-behavior-runtime-verification.md) | 2 → 4 → 6; peak 263% against the 70% target; all 6 replicas became Ready | Yes; proven recommendation duration ≥270s (anchor 13:09:02Z at 6 replicas and 4%, desiring the floor; last old-count sample 13:13:33Z; decision observed 13:13:49Z), then 6 → 5 → 4 → 3 → 2 | 6 of 6 transitions carry the HPA's own rescale evidence; `ScalingActive=True` throughout, Ready ≥2, zero restarts, cleanup confirmed |
| `telemetry-processor` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/telemetry-processor/hpa-behavior-runtime-verification.md) | 2 → 3; peak 358% against the 70% target; all 3 replicas became Ready | Yes; proven recommendation duration ≥277s (anchor 14:01:58Z at 3 replicas and 37%, desiring the floor; last old-count sample 14:06:36Z; decision observed 14:06:47Z), then 3 → 2 | 2 of 2 transitions carry the HPA's own rescale evidence; `ScalingActive=True` throughout, Ready ≥2, zero restarts, cleanup confirmed |

Both reports were generated by `validate-autoscaling-behavior.ps1 -IncludeScaleDown` from tested code revision `94213047802debcbb8ee5e3ed2081174dc1e1034`, each on a clean working tree. Re-running the harness on each cluster the platform is deployed to is the intended use, with the generated report committed next to the manifest it describes.

The ingestion run used 6 concurrent request loops and reached the six-replica ceiling with every replica Ready. Telemetry used a 150 events/s burst for 90 seconds, then the bounded 60 events/s maintenance rate, and reached its three-replica partition ceiling with every replica Ready. Every scale event in both runs is timestamped from the scale subresource's desired count and correlated with the HPA's own `desiredReplicas` and an advancing `lastScaleTime`, and the scale-down verdicts rest on the conservative bound rather than on the anchor-to-decision spacing (287s for ingestion, 289s for telemetry) that the samples merely suggest.
