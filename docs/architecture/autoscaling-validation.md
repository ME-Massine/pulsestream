...
`scripts/validate-autoscaling-behavior.ps1` records a timeline of samples — the scale subresource's desired replica count, the Deployment's status and ready counts, the HPA's own `desiredReplicas`/`lastScaleTime`/conditions, CPU utilization against target, every other metric the HPA is configured with, and container restarts — across three phases (idle, under load, after load), then applies these rules. They live in `scripts/lib/PulseStreamAutoscalingBehavior.psm1` as pure functions, so `scripts/tests/test-autoscaling-behavior-analysis.ps1` exercises each of them against synthetic timelines with no cluster involved.

The replica concepts stay separate on purpose. The scale subresource's `spec.replicas` is where the HPA writes its decision, so it moves **at the decision**; the Deployment's `status.replicas` moves when the rollout catches up, which can be many seconds later, and `readyReplicas` later still. Scale events are therefore timestamped from the first sampled transition of the scale subresource's desired count — never from a delayed workload-status change — while status and ready counts keep their own job of proving the requested capacity actually materialized.
...
| Every replica transition was timed from the scale subresource | Decision timing. Both samples bounding a transition must carry the scale subresource read, so the event is timestamped where the HPA writes its decision. A transition visible only through `status.replicas` is timestamped when the *rollout* caught up, and that lag would be handed to the stabilization bound as if it were stabilization time — so a run that lost the read around a transition is rejected rather than judged on the workload's clock. The sampler treats a scale object without `spec.replicas` as a hard failure for the same reason |
| Every replica transition correlates with the HPA's own rescale evidence | Attribution. A transition is credited to the HPA only when the HPA's `status.desiredReplicas` equals the transition's new count **and** its `status.lastScaleTime` advanced across the transition — the controller's own statement that it changed the target's scale in that interval. `lastScaleTime` values are compared only against each other, never against the sampler's clock, so clock skew cannot manufacture or destroy an advance; and because the HPA can rescale between one sample's HPA read and its scale read, the evidence is also accepted one sample late, but only while the scale target still holds the transition's count. A transition without that evidence — a manual resize, a rollout, another controller — fails the run, because every other assertion reads the replica numbers as the autoscaler's work |
...
The samples never *measure* the stabilization window; they *bound* it. The recommendation turned over somewhere inside the gap that ends at the anchor sample, and the scale-down decision happened somewhere after the last sample that still showed the old desired count. The true window therefore lies inside an interval, and which end of that interval the verdict judges is the whole question:

- **Proven duration** — anchor sample to the last sample still showing the old desired count. The recommendation was observed to hold across every sample of that stretch, and the decision came strictly *after* its end, so the true window is strictly **longer** than this bound.
...
The same rule applies *inside* the stretch. A sample proves the recommendation at the instant it was taken and says nothing about the interval after it, so two recommending samples 299 seconds apart do not prove 299 seconds of recommendation: a load spike in between would have raised the recommendation and restarted the controller's window without this run ever seeing it. The proven stretch is therefore trimmed forward past any spacing wider than the harness treats as continuous observation — twice the *requested* sampling interval, minimum 60s, again never widened by what the run actually did — and what survives the trim is what the verdict judges. A run that stalled for 299 seconds mid-window is left proving nothing rather than credited with the stall; a run that stalled early and then watched the recommendation continuously still passes on the well-sampled remainder, with the trim and its reason printed in the report and in the verdict either way.

What remains of the sampling grace is the *fixed* quantization allowance of the **requested** interval — twice the requested sampling interval, minimum 30s — because even a perfectly compliant window loses up to one interval at each end of the proven bound. Nothing the run actually did widens it (`Get-ScaleDownSamplingGrace` owns this rule so the tests can hold it to it offline), and it is capped at half the stabilization window: past that the requested sampling is too coarse to say whether the window held at all, and preflight rejects the configuration before load starts. Every run report records the requested interval, the widest observed gap, the gap before the scale-in recommendation, and the fixed grace the verdict used.
...
| :--- | :--- | :--- | :--- | :--- |
| `ingestion-service` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/ingestion-service/hpa-behavior-runtime-verification.md) | 2 → 4 → 6; peak 181% against the 70% target; all 6 replicas became Ready | Yes; proven recommendation duration ≥270s (anchor 02:32:00Z at 6 replicas and 20%, desiring the floor; last old-count sample 02:36:30Z; decision observed 02:36:47Z), then 6 → 5 → 4 → 3 → 2 | 6 of 6 transitions carry the HPA's own rescale evidence; `ScalingActive=True` throughout, Ready ≥2, zero restarts, cleanup confirmed |
| `telemetry-processor` | [`hpa-behavior-runtime-verification.md`](../../infrastructure/kubernetes/telemetry-processor/hpa-behavior-runtime-verification.md) | 2 → 3; peak 105% against the 70% target; all 3 replicas became Ready | Yes; proven recommendation duration ≥272s (anchor 02:54:23Z at 3 replicas and 34%, desiring the floor; last old-count sample 02:58:56Z; decision observed 02:59:07Z), then 3 → 2 | 2 of 2 transitions carry the HPA's own rescale evidence; `ScalingActive=True` throughout, Ready ≥2, zero restarts, cleanup confirmed |

Both reports were generated by `validate-autoscaling-behavior.ps1 -IncludeScaleDown` on a clean working tree: ingestion from tested code revision `2bfe059128be432a3a605aad9ff1c41442e512d2`, telemetry from `f40967afbfdd7eab8504924dc0296f73b5c5016c`, which is the same harness code plus the commit that recorded the ingestion report. Re-running the harness on each cluster the platform is deployed to is the intended use, with the generated report committed next to the manifest it describes.

The ingestion run used 6 concurrent request loops and reached the six-replica ceiling with every replica Ready. Telemetry used an 800 events/s burst for 90 seconds, then the bounded 300 events/s maintenance rate, and reached its three-replica partition ceiling with every replica Ready. Every scale event in both runs is timestamped from the scale subresource's desired count and correlated with the HPA's own `desiredReplicas` and an advancing `lastScaleTime`, and the scale-down verdicts rest on the conservative bound rather than on the anchor-to-decision spacing (286s for ingestion, 283s for telemetry) that the samples merely suggest.
