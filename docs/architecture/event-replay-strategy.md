...
- **Bounded.** Once the listener has drained the backlog present when it was triggered it goes idle and stops itself, so it does not stay running to sweep up future dead-letter records. Pre-assignment idle events are ignored so a slow consumer-group join is not mistaken for a completed scan; the configurable idle window defaults to 30 seconds. `start`/`stop` are idempotent, and the listener also stops itself on a failed republish (no-data-loss policy from #124), so the record is redelivered on the next `start`.
